...
const gulpHeadLine = require("./gulplib/gulpwrite");
...
const USER_COMPONENT_TEMPLATE_FILE_PATH =
  "seed/user-components-templates/user-component-basic.js.tpl";
...
const USER_PAGE_TEMPLATE_FILE_PATH =
  "seed/user-pages-templates/user-page-basic.js.tpl";
...
gulp.task("create-user-components", function (done) {
  gulpHeadLine.taskName("create-user-components");
  ...
  let userComponents = [];
  _componentBuild(
    userComponentsJSONFilePaths,
    USER_COMPONENT_OWN_CSS,
    USER_COMPONENT_DIST,
    userComponents
  );
...
gulp.task("create-user-pages", function (done) {
  gulpHeadLine.taskName("create-user-pages");
...
gulp.task("create-app", function (done) {
  gulpHeadLine.taskName("create-app");
  const target = "App";
...
gulp.task("create-menu", function (done) {
  gulpHeadLine.taskName("create-menu");
  const target = "Menu";
  if (
    FS.existsSync(MENU_TEMPLATE_PATH) === false ||
    FS.existsSync(MENU_CONFIG_JSON) === false
  ) {
    if (FS.existsSync(`${APP_COMPONENTS_DIST}${target}.tsx`))
      FS.unlinkSync(`${APP_COMPONENTS_DIST}${target}.tsx`);
    if (FS.existsSync(`${APP_COMPONENTS_DIST}${target}.css`))
  ...
  let menuHeaderTitle =
    menuConfigJSON.header && menuConfigJSON.header.strTitle
      ...
      : "Ionic Seed App";
  menuTemplateFileBuffer = _replaceTag(
    "APP_HEADER",
    menuHeaderTitle,
    menuTemplateFileBuffer
  );
  let signOutCaption =
    menuConfigJSON.signOut && menuConfigJSON.signOut.strCaption
      ? menuConfigJSON.signOut.strCaption
      : "Sign Out";
  menuTemplateFileBuffer = _replaceTag(
    "APP_MENU_SIGNOUT",
    signOutCaption,
  ...
  menuTemplateFileBuffer = _replaceTag(
    "APP_MENU",
    JSON.stringify(buildMenu).replace(/\"/g, ""),
    menuTemplateFileBuffer
  );
  menuTemplateFileBuffer = _replaceTag(
    "APP_MENU_ICONS",
  ...
  let menuBottomParameters =
    menuConfigJSON.menuBottom && menuConfigJSON.menuBottom.parameters
      ? menuConfigJSON.menuBottom.parameters.join("\n")
      : "";
  menuTemplateFileBuffer = _replaceTag(
    "APP_MENU_BOTTOM_PARAMETER",
    menuBottomParameters,
    menuTemplateFileBuffer
  );
  let tags = [];
  if (menuConfigJSON.menuBottom)
    _htmlTagRecursive(menuConfigJSON.menuBottom, tags);
  menuTemplateFileBuffer = _replaceTag(
    "APP_MENU_BOTTOM",
    _tagToHtml(tags),
    menuTemplateFileBuffer
...
let _componentBuild = function (
  componentConfigJSONFilePaths,
  cssSeedDirectory,
  cssDist,
  buildComponents
) {
  _.forEach(componentConfigJSONFilePaths, (componentConfigJSONFilePath) => {
    ...
    _componentMethodRecursiveWithOverlapCheck(
      componentConfigJSON,
      componentMethods
    );
    ...
    userComponentSet["ownCss"] = _ownCss(
      cssSeedDirectory,
      cssDist,
      path.basename(componentConfigJSONFilePath).replace("json", "css"),
      componentName
    );
      ...
      fetch.apis.forEach((api) => {
        ...
        if (api.uri === gulpConst.slsFrontApiUri()) {
          if (api.config && api.config.path) {
            ...
            throw new Error(
              "Could not find fetch -> apis[] -> config -> path."
            );
          }
        }
      });
  ...
  if (_.isUndefined(sauceJSON.tags) === false) {
    _.forEach(sauceJSON.tags, (component) => {
      let openTagSet = {
        open: component.tag,
        props:
          _componentProperties(component) + " " + _componentEvents(component),
      };
      let closeTag = component.tag;
      if (_.isUndefined(component.type) === false)
        openTagSet["type"] = component.type;
      if (_.isUndefined(component.close) === false) closeTag = component.close;
      if (_.isUndefined(component.content) === false)
        openTagSet["content"] = component.content;
      if (_.isUndefined(component.rawContent) === false)
        openTagSet["rawContent"] = component.rawContent;
      if (_.isUndefined(component.single) === false)
        openTagSet["single"] = component.single;
      ...
      if (_.isUndefined(component.child) === false) {
        ...
        let closeTagSet = {};
        if (_.isUndefined(component.type) === false)
          closeTagSet["type"] = component.type;
        if (_.isUndefined(component.tag) === false)
          closeTagSet["close"] =
            _.isUndefined(component.close) === false
              ? component.close
              : component.tag;
        if (_.isUndefined(component.noCR) === false)
          closeTagSet["noCR"] = component.noCR;
        if (_.isUndefined(component.contentAT) === false)
          closeTagSet["contentAfterTag"] = component.contentAT;
        if (_.isUndefined(component.rawContent) === false)
          closeTagSet["rawContent"] = component.rawContent;
...
let _componentMethodRecursiveWithOverlapCheck = function (
  sauceJSON,
  componentMethods
) {
  if (_.isUndefined(sauceJSON.tags) === false) {
    _.forEach(sauceJSON.tags, (component) => {
      if (_.isUndefined(component.componentMethod) === false) {
        _.forEach(componentMethods, (existComponentMethod) => {
          _.forEach(
            existComponentMethod.states,
            (overlapCheckComponentStat) => {
              if (
                _.find(component.componentMethod.states, (v) => {
                  return v.name === overlapCheckComponentStat.name;
                })
              ) {
          ...
          _.forEach(
            existComponentMethod.methods,
            (overlapCheckComponentMethod) => {
              if (
                _.find(component.componentMethod.methods, (v) => {
                  return v.name === overlapCheckComponentMethod.name;
                })
              ) {
...
let _htmlTag = function (tag) {
  ...
  if (tag.open) {
    result =
      `<${tag.open}${tag.props}${tag.single ? " /" : ""}>` +
      (tag.content ? tag.content : "");
  } else if (tag.close) {
    result =
      (tag.single ? "" : `</${tag.close}>`) +
      (tag.contentAfterTag ? tag.contentAfterTag : "");
  }
  return result;
};
...
let _typePackage = function (component) {
  if (
    _.isUndefined(component.type) === false &&
    component.type.toLowerCase() === "package"
  ) {
    return true;
  }
  return false;
...
let _typeCss = function (component) {
  if (
    _.isUndefined(component.type) === false &&
    component.type.toLowerCase() === "css"
  ) {
    return true;
  }
  return false;
...
let _dedupeImportComponents = function (component, userPageImportComponents) {
  ...
  if (_.isUndefined(userPageImportComponents[component.from])) {
    ...
    if (_.isUndefined(existGroupComponents[component.name]))
      existGroupComponents.push(component.name);
    userPageImportComponents[component.from] = existGroupComponents;
  }
};

let _dedupeDefaultImportComponents = function (
  component,
  userPageDefaultImportComponents
) {
  if (_typeDefault(component) === false) return;
  let importSet = { name: component.name, from: component.from };
  if (
    _.find(userPageDefaultImportComponents, (v) => {
      return v.name === component.name;
    })
  ) {
  ...
  for (let importFrom in userPageImportComponents) {
    ...
    _.forEach(components, (component) => {
      commaSeparatedComponents +=
        (commaSeparatedComponents ? " ," : "") + component;
    });
    result +=
      (result ? "\n" : "") +
      "import { " +
...
let _importDefaultImportComponentDeclaration = function (
  userPageDefaultImportComponents
) {
  let result = "";
...
let _createUserComponentFile = function (userComponents, prefix = "") {
  _createComponentFile(
    userComponents,
    USER_COMPONENT_TEMPLATE_FILE_PATH,
    USER_COMPONENT_DIST,
    prefix
  );
};
...
let _createComponentFile = function (
  targetComponents,
  templateFilePath,
  componentDist,
  prefix = ""
  ...
  targetComponents.forEach((componentSet) => {
    ...
    let userComponentImportDeclaration = _importComponentDeclaration(
      importComponents
    );
    fileBuffer = _replaceTag(
      "IMPORT_COMPONENTS",
      userComponentImportDeclaration,
      fileBuffer
    );
    let userComopnentDefaultImportDeclaration = _importDefaultImportComponentDeclaration(
      defaultImportComponents
    );
    fileBuffer = _replaceTag(
      "DEFAULT_IMPORT_COMPONENTS",
      userComopnentDefaultImportDeclaration,
      fileBuffer
    );
    let userImportCssDeclaration = _importImportCssDeclaration(importCss);
    fileBuffer = _replaceTag(
      "IMPORT_CSS",
      userImportCssDeclaration,
      fileBuffer
    );
    let userImportOwnCssDeclaration = componentSet.ownCss
      ? componentSet.ownCss.import
      : "";
    fileBuffer = _replaceTag(
      "IMPORT_OWN_CSS",
      userImportOwnCssDeclaration,
      fileBuffer
    );
    let stateInterface = _readTemp(componentSet.name, TEMP_EXT_STATE_INTERFACE);
    fileBuffer = _replaceTag(
      "STATE_INTERFACE",
      stateInterface ? `interface State {${stateInterface}}` : "",
      fileBuffer
    );
...
let _componentFetchGet = function (componentName, fetch) {
  ...
  _.forEach(fetch.apis, (api) => {
    ...
    fetchApi +=
      (fetchApi ? ", " : "") +
      `() => fetch.get<${api.responseTypeName}>('${
        api.uri
      }${_componentFetchQueryStrings(api.queryStrings)}'${
        api.init ? ", " + api.init : ""
      })`;
  });
  _componentFetchAppendTemp(componentName, type, fetch);
  return _componentFetchDataReplacement(
    templateFetchDataFilePath,
    fetchApi,
    fetch
  );
};

let _componentFetchQueryStrings = function (queryStrings) {
  let functionName = "_componentFetchQueryStrings()";
  if (!queryStrings) return "";
  let query = "";
  _.forEach(queryStrings, (queryString) => {
    query += `${query ? "&" : ""}${queryString.parameter}=${queryString.value}`;
  });
  return (query ? "?" : "") + query;
};
...
let _componentFetchPost = function (componentName, fetch) {
  ...
  _.forEach(fetch.apis, (api) => {
    if (!api.postTypeName)
      throw new Error(
        `Could not find fetch -> apis[] -> postTypeName@${fetch.name}.`
      );
    if (!api.postType)
      throw new Error(
        `Could not find fetch -> apis[] -> postType@${fetch.name}.`
      );
    if (!api.postBody)
      throw new Error(
        `Could not find fetch -> apis[] -> postBody@${fetch.name}.`
      );
    type +=
    ...
    fetchApi +=
      (fetchApi ? ", " : "") +
      `() => fetch.post<${api.postTypeName}, ${api.responseTypeName}>('${
        api.uri
      }'${api.init ? ", " + api.init : ""}, ${api.postBody})`;
...
let _componentFetchAppendTemp = function (componentName, type, fetch) {
  ...
  _appendTemp(
    componentName,
    TEMP_EXT_STATE_INTERFACE,
    _componentFetchStateInterface(fetch)
  );
};

let _componentFetchDataReplacement = function (
  templateFetchDataFilePath,
  fetchApi,
  fetch
) {
  ...
  fetchDataBuffer = _replaceTag(
    "ARGS",
    _componentFetchMethodArgs(fetch),
    fetchDataBuffer
  );
  fetchDataBuffer = _replaceTag("FETCH", fetchApi, fetchDataBuffer);
  fetchDataBuffer = _replaceTag(
    "RETURN_TYPE",
    _componentFetchResponseType(fetch),
    fetchDataBuffer
  );
  fetchDataBuffer = _replaceTag(
    "API_COUNT",
    _componentFetchApiCount(fetch),
    fetchDataBuffer
  );
  fetchDataBuffer = _replaceTag(
    "SET_STATE",
    _componentFetchSetState(fetch),
    fetchDataBuffer
  );
  fetchDataBuffer = _replaceTag(
    "CODE_FIRST",
    fetch.codeFirst ? fetch.codeFirst : "",
    fetchDataBuffer
  );
  fetchDataBuffer = _replaceTag(
    "CODE_LAST",
    fetch.codeLast ? fetch.codeLast : "",
    fetchDataBuffer
  );
...
let _componentLifeCycleMethod = function (componentSet) {
  ...
  if (_isSet(componentSet, "lifeCycleMethods", functionName) === false)
    return "";
  ...
  _.forEach(componentSet.methods, (componentMethod) => {
    ...
    _.forEach(componentMethod.methods, (eachComponentMethod) => {
      result += _userFunction(
        eachComponentMethod.methodType,
        eachComponentMethod
      );
    });
  });
...
let _userFunction = function (template, componentMethod) {
  ...
  methodTemplate = _replaceTag(
    "args",
    _userMethodArgs(componentMethod.args),
    methodTemplate
  );
...
let _componentRenderBeforeReturn = function (componentSet) {
  ...
  if (_isSet(componentSet, "renderBeforeReturn", functionName) === false)
    return "";
